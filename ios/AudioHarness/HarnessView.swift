...
struct HarnessView: View {
    ...
    private var tuningSection: some View {
        Section("Renderer diagnostics (applied engine settings)") {
            Text("Read-only. Defaults come from SweepRendererSettings.listeningTest, not a second UI copy.")
                .font(.footnote)
                .foregroundStyle(.secondary)
            ForEach(Array(model.rendererDiagnosticLines.enumerated()), id: \.offset) { _, line in
                Text(line)
